...
Discord-native DOJ document filing. Members pick a document type from a dropdown, answer questions one at a time over DM, and the finished document goes to a staff review forum for Approve/Deny (with optional multi-signer handoff) before an explicit "Make Public" step publishes it to a GitHub Pages site.

**Features:**
- 4-step setup wizard (document channel, review forum, approval role, site repository)
